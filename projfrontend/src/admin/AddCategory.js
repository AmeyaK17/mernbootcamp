...
const AddCategory = () => {
    ...
    const {user, token} = isAuthenticated(); //extract info when it runs
    ...
    const onSubmit = event => {
        ...
        createCategory(user._id, token, {name})//{name} so that it can be passed on as an object for the JSON.stringify() method.
            .then(data => {
                if(data.error){
                    setError(true)
                }
